-- Canonical Postgres baseline snapshot. Runtime initialization must not mutate Postgres schema directly after this migration is applied.
...
create table if not exists kpi_definitions (
    ...
    version text, calculation_version text default '1.0', data_owner_role text, effective_from text, effective_to text, active text, created_on text, updated_on text
);
...
-- Phase 6: immutable audit and scoped QR verification support are validated
-- in subsequent migrations; this baseline remains the canonical table snapshot.
