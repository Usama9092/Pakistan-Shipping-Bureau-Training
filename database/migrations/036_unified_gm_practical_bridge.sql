-- Unified PSB bridge migration.
-- Idempotently ensures both branch-specific 035 feature sets exist, allowing
-- upgrades from either the GM Executive or Practical/Witness lineage.
...
-- GM executive workspace role + personal watchlist.
insert into roles(role_id, role_name, description, status, created_on, updated_on)
select 'ROLE-GM', 'GM', 'Global Manager executive governance, organization-wide oversight and system administration without implicit technical approval authority.', 'Active', current_date::text, current_timestamp::text
where not exists (select 1 from roles where role_name='GM');

create table if not exists gm_watchlist (
    watch_id text primary key, gm_user_id text not null, record_type text not null, record_ref text not null,
    title text, risk_level text, status text, due_date text, route text, added_on text,
